...
export default function App() {
  ...
  const [hudData,       setHudData]       = useState({ takeaways: [], objectives: [] });
  ...
  useEffect(() => {
    const handleMessage = (event) => {
      ...
      if (
        ...
        setHudData((prev) => {
          const updated = prev.objectives.map((obj, i) => {
  ...
  const handleGenerate = useCallback(async ({ topic, level, sourceType, documentData }) => {
    setActiveTopic(topic);
    setActiveLevel(level);
    setActiveDocData(documentData || null);
    setSourceType(sourceType || 'topic');
    setAppState('loading');
    setErrorMsg('');
    ...
    try {
      ...
      if (sourceType === 'document' || sourceType === 'combined' || documentData) {
  ...
  const handleSelectTopicFromSlide = (topicTitle) => {
  ...
  return (
    <div className="min-h-screen flex flex-col relative overflow-x-hidden">
          ...
          <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8 animate-fade-in">
            ...
            {appState === 'error' && (
              <div className="px-4 sm:px-6 max-w-4xl mx-auto mb-8">
                <div className="glass-card p-6 flex flex-col sm:flex-row items-start sm:items-center gap-4"
                  ...
                  <button className="control-btn" style={{ color: '#dc2626', borderColor: 'rgba(239, 68, 68, 0.30)' }}
            ...
            {appState === 'playing' && (
              <>
                ...
                />
                <HUDDrawer
                  takeaways={hudData.takeaways}
                  objectives={hudData.objectives}
                  topic={activeTopic}
                />
              </>
            )}
            ...
            {appState === 'idle' && (
              <div className="flex flex-col items-center justify-center pb-12 px-4 text-center">
                ...
                <div className="grid sm:grid-cols-3 gap-4 max-w-3xl w-full mt-8">
                  {[
                    { Icon: Zap,          color: '#5680E9', bg: 'rgba(86, 128, 233, 0.10)', border: 'rgba(86, 128, 233, 0.25)', title: 'Instant Games',       desc: 'AI synthesizes interactive game code in seconds from any prompt' },
                    { Icon: Presentation, color: '#1f8ebd', bg: 'rgba(90, 185, 234, 0.14)', border: 'rgba(90, 185, 234, 0.30)', title: 'PDF & PPT Grounding', desc: 'Parses lecture slides directly into questions with side-by-side view' },
                    { Icon: Brain,        color: '#8860D0', bg: 'rgba(136, 96, 208, 0.12)', border: 'rgba(136, 96, 208, 0.25)', title: 'Calibrated Levels',  desc: 'Adapts vocabulary & depth from Elementary to University' },
      ...
      <footer className="py-8 px-6 mt-auto relative z-10 transition-colors"
        style={{
        ...
        <div className="max-w-6xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4 text-xs text-[#7c8ba8]">
          ...
          <div className="flex items-center gap-6">
            ...
            <button 
              onClick={() => { setActivePage('ai-agent'); window.scrollTo({ top: 0, behavior: 'smooth' }); }} 
              className="hover:text-[#5680E9] font-pixel transition-colors">
              AI Agent
            </button>
            <button 
